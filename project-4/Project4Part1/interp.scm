    ...
    (lambda (exp env)
      (cases expression exp
        ...
        (proc-nested-exp (var count name body)
                         (proc-val (nested-procedure var body env count name)))

        (call-nested-exp (rator rand count)
                  (let ((proc (expval->proc (value-of rator env)))
                        (arg (value-of rand env))
                        (counter (value-of count env)))
                    (apply-procedure proc (cons arg counter))))
  ...
  (define apply-procedure
    (lambda (proc1 arg)
      (cases proc proc1
        ...
        (nested-procedure (var body saved-env count name)
         (recursive-displayer name (expval->num (cdr arg)))
          (value-of body (extend-env count (cdr arg) (extend-env var (car arg) saved-env))))
