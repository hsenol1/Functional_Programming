(module tests mzscheme
  ...
  (define test-list
    '(
      ...
      ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
     (fibonacci-test
  "letrec fib(n) = if zero?(n) then 0 else if zero?(-(n,1)) then 1 else -( (fib -(n,1)) , -(0, (fib -(n,2))) ) in (fib 5)" 
  5)

     (natural-number-sum
  "letrec sum(n) = if zero?(n) then 0 else -( (sum -(n,1)), -(0, n)) in (sum 5)"
  15)


   (random-mix-procs
    "let proc1 = proc(x) -(x,-5)
      in let proc2 = proc (y) -(y,4)
      in let proc3 = proc (z) -(-(z,-9), (proc2 6))
      in (proc1 (proc3 1))" 13)
